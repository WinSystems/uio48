...
int read_bit(int chip_number, int bit_number)
{
    ...
    if ((chip_number < 1 || chip_number > MAX_CHIPS) || (bit_number < 1 || bit_number > 48)) //checks for valid parameters
        return -2;       
...
int enab_int(int chip_number, int bit_number, int polarity)
{
    ...
    if ((chip_number < 1 || chip_number > MAX_CHIPS) || (bit_number < 1 || bit_number > 48) || (polarity < RISING || polarity < FALLING)) //checks for valid parameters
                return -2;
...
int get_int(int chip_number)
{
    ...
    if ((chip_number < 1 || chip_number > MAX_CHIPS)) //checks for valid parameters
                return -2;
...
int wait_int(int chip_number, unsigned int timeout)
{
   ...
   // if(check_handle(chip_number))   /* Check for chip available */
    ...
    c = ioctl(handle[chip_number], IOCTL_WAIT_INT, 10000);
...
int clr_int_id(int chip_number, int port_number)
{
    ...
    if ((chip_number < 1 || chip_number > MAX_CHIPS) || (port_number > 5 || port_number < 0)) //checks for valid parameters
                return -2;
